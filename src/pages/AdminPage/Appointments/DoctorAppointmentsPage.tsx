      ...
      <div className="overflow-y-auto flex flex-col gap-3 pr-1">
        {filtered.length === 0 ? (
          ...
          filtered.map((appt, idx) => {
            ...
            return (
              <div key={appt.appointment_id} className="w-[408px] h-[68px] flex items-center bg-white border border-gray-200 rounded-lg shadow-sm">
...
const DoctorAppointmentsPage: React.FC<DoctorAppointmentsPageProps> = ({ doctorName }) => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const doctorRes = await axios.get(`https://medical-assistant1.onrender.com/doctors?name=${doctorName}`);
        const doctor = doctorRes.data.doctors.find((d: any) => d.name === doctorName);
        const doctorId = doctor?.id;
        if (!doctorId) return;

        const date = selectedDate.format("YYYY-MM-DD");
        const apptRes = await axios.get<{ appointments: Appointment[] }>(
          `https://medical-assistant1.onrender.com/appointments?doctor_id=${doctorId}&date=${date}`
        );

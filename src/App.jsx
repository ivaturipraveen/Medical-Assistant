...
export default function App() {
  ...
  useEffect(() => {
    if (view === 'doctors') {
      const fetchDoctors = async () => {
        ...
        try {
          const data = await fetchWithRetry('http://localhost:8000/doctors');
      ...
      const fetchDepartments = async () => {
        ...
        try {
          const response = await fetchWithRetry('http://localhost:8000/categories');
  ...
  useEffect(() => {
    if (view === 'patients') {
      const fetchPatients = async () => {
        ...
        try {
          const data = await fetchWithRetry('http://localhost:8000/patients');
  ...
  useEffect(() => {
    ...
    const fetchPatientsCount = async () => {
      try {
        const data = await fetchWithRetry('http://localhost:8000/patients/count');
  ...
  useEffect(() => {
    if (view === 'dashboard' && !showLogin) {
      const fetchDashboardStats = async () => {
        ...
        try {
          const data = await fetchWithRetry('http://localhost:8000/dashboard/stats');
      ...
      const fetchDashboardData = async () => {
        ...
        try {
          const [
            ...
            fetchWithRetry('http://localhost:8000/dashboard/appointments-by-department'),
            fetchWithRetry('http://localhost:8000/dashboard/weekly-distribution'),
            fetchWithRetry('http://localhost:8000/dashboard/doctor-workload'),
            fetchWithRetry('http://localhost:8000/dashboard/age-distribution'),
          ]);
  ...
  useEffect(() => {
    const fetchAllAppointments = async () => {
      ...
      try {
        const response = await fetchWithRetry('http://localhost:8000/appointments');
